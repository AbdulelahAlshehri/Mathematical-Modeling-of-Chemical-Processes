...
for i=1:30
    Qo=random('normal',3.2e-5,0.5e-5);  %Choose a final slope of 9
                              

%Solve an ODE
u_init=[1  Qo]; %set final value of theta and final slope (guess)
xspan=[1 0.001]; %set the range of the x variable backward (scaled problem) % 0.01 was used to avoid dividing by zero in myfun
